...
# 
# x = 2018-1940
# vac_2018<-rowSums(vc3d[, x, ]*pop3d[, x, ], na.rm = TRUE)/rowSums(pop3d[, x, ], na.rm = TRUE)
# pop_2018<-rowSums(pop3d[, x, ], na.rm = TRUE)
# 
# vac_pop_df<-data.frame("ISO" = gsub("\\_.*", "", names(vac_2018)),
#                    "SPID" = as.character(sapply(names(vac_2018), function(x) paste(unique(unlist(strsplit(x, "_"))), collapse = "_"))),
#                    "adm1_name" = as.character(endemiczone$NAME_1[which(as.character(endemiczone$SPID) %in% gsub("_", "", as.character(vac_pop_df$SPID)))]),
#                    "vac" = as.numeric(vac_2018),
#                    "pop" = as.numeric(pop_2018))
# 
# 
# 
# 
# # 
# 
# write.csv(vac_pop_df, "vac_pop_2018.csv", row.names = FALSE)
# 
# 


# vac_pop<-rowSums((vc3d*pop3d), na.rm = TRUE)/rowSums(pop3d, na.rm = TRUE)
